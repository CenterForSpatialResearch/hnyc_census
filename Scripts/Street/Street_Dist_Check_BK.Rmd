...
```{r, message = FALSE}
# load datasets
sample <- read_csv(here::here("Data/us1910m_usa_BK_sample100k.csv"))
edict <- read_csv(here::here("Data/full_bk_dict.csv"))
...
```
615 (75.5%) records have a perfect match.
...
table(n_match_within_algo$n_diff_match) %>%
  ...
  kable_styling(full_width = FALSE)
```
We know that 615 records have perfect matches between all results from the string algorithms and the clean string. Additionally, 725 - 615 = 110 (13.5%) records have all string algorithms giving the same result--suggesting that this is a problem with the cleaning rather than matching. To verify this, the records that are not a perfect match, but have all the same results are shown below:
...
```
It does seem that for such records, we can confidently replace the string with the matched string. At this point, perfect matches and records with identical matches comprise 89% of the records, i.e. we can confidently say at least 89% of our records are accurately matched.

Another 90 (11.0%) records have different matches depending on string algorithm. These are shown below.
```{r}
match_dict %>%
  replace(is.na(.), 0) %>%
...
match_dict_imp %>%
  ...
  nrow()
```
This represents 92.9% of the sample. They are again shown here:
...
In the MN sample, it was found that there are a few records (10, 1.6%) were 2 string algorithms return a perfect match. This is important to examine because if only 2 string algorithms gave a true, correct match, the modal match would not identify this. However, none exist for Brooklyn.
...
match_dict_imp %>%
  ...
  kable() %>% kable_styling() %>% scroll_box(height = "300px")
```
There are 10 such rows, i.e. 1.2% of the sample.
...
match_dict_imp %>%
  ...
  kable() %>% kable_styling() %>% scroll_box(height = "300px")
```
There are 25 such records (3.1%). Again, there are instances where one of the non-mode matches seems correct, and others where the mode is correct. Nonetheless, fill down cleaning is recommended instead, especially due to the small size of this issue.

### Result 4: No matches at all
Lastly, using this method returns records (23 records, 2.8%) that have no matches at all. These are shown below:
...
na_2 <- na_match_in + 2
na_3 <- na_match_in + 3
na_match_in <- c(na_match_in, na_match_1, na_match_2, na_match_3, na_1, na_2, na_3) %>% unique() %>% sort() %>% .[na_match_in >= 0]
...
The script worked even better for the Brooklyn sample compared to Manhattan!