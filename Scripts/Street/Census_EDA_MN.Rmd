...
```{r, echo = FALSE}
head(sample) %>% kable %>% kable_styling()
```
...
head(household[na_street, c("hh_seq", "street_add")], 30) %>% kable %>% kable_styling()
```

From a brief check of the dataset, it seems that the household sequences are generally consecutive for each set of addresses, suggesting that filling down from one household person to another is viable.
