...
## 6.2. Address Sequences
Currently, fill down looks at the 3 rows above and below missing records. Fill down could be improved by examining the average length of a sequence.

```{r}
add_seq_ED <- match_dict_imp %>%
  bind_cols(ED2 = as.factor(as.numeric(match_dict_imp$ED))) %>%
  split(., .$ED2)

add_seq_breaker <- function(df) {
  df$next_match <- df$best_match %>% unlist() %>%
    append(NA_character_, after = nrow(df)) %>%
    .[-1]
  
  if (nrow(df) > 1) {
    seq <- df %>%
      filter(best_match != next_match) %>%
      select(index) %>%
      bind_cols(seq = 1:nrow(.))
    
    df <- left_join(df, seq, by = "index") %>%
      fill(seq, .direction = "up")
  
    df[nrow(df), "seq"] <- 0
    df <- df %>%
      fill(seq, .direction = "up") %>%
      mutate(seq = paste(ED, seq, sep = "_"))
      
  } else if (nrow(df) == 1) {
    df$seq <- paste(df$ED, "1", sep = "_")
  }
  
  df <- select(df, - c("ED2", "next_match"))
  return(df)
}
...
add_seq %>% ungroup() %>%
  ...
  ggplot(aes(x = length)) +
    geom_bar() +
    geom_vline(aes(xintercept = mean(length), color = "red")) + 
    labs(title = "Distribution of Sequence Lengths") +
    scale_color_discrete(name = "", labels = c("Mean Length"))
```

```{r}
seq_length_ED <- add_seq %>% ungroup() %>%
  group_by(ED, seq) %>%
  summarise(length = n()) %>%
  group_by(ED) %>%
  summarise(n_seq = n(), mean_length = mean(length))
```

```{r, warning=FALSE, message=FALSE, echo=FALSE}
library(sf)
ED_geom <- st_read(here::here("data/Manhattan_1910.shp")) %>%
...
binpal_len <- colorBin("YlOrRd", ED_geom$mean_length, c(0, 1.5, 2, 3, 4.25))
binpal_seq <- colorQuantile("YlOrRd", ED_geom$n_seq)

popup <- paste("<b>ED:</b>", ED_geom$ED, "<br>",
               "<b>Mean Length:</b>", round(ED_geom$mean_length, 2), "<br>",
               "<b>No. of Sequences:</b>", ED_geom$n_seq)
...
leaflet(ED_geom) %>%
  ...
  addPolygons(color = "#444444", weight = 1, smoothFactor = 0.5,
    ...
    fillColor = ~ binpal_len(ED_geom$mean_length),
    popup = popup,
    group = "Mean Length") %>%
  addLegend("bottomright", pal = binpal_len, values = ~ mean_length,
    title = "Mean Length of Sequences in EDs",
    opacity = 1,
    group = "Mean Length") %>%
  ...
  addPolygons(color = "#444444", weight = 1, smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 0.5,
    fillColor = ~ binpal_seq(ED_geom$n_seq),
    popup = popup,
    group = "Number of Sequences") %>%
  addLegend("bottomright", pal = binpal_seq, values = ~ n_seq,
    title = "Number of Sequences in EDs",
    opacity = 1,
    group = "Number of Sequences") %>%
  
  # toggle
  addLayersControl(overlayGroups = c("Mean Length", "Number of Sequences"), 
               options = layersControlOptions(collapsed = T)) %>%
  hideGroup("Number of Sequences")
```
