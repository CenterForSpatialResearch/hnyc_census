---
title: "Implementing Spelling Check within Street Cleaning Script"
author: "Gerald Lee"
date: "11/18/2019"
...
```{r}
df2 <- read_csv("us1910m_usa_sample100k.csv") # Load Data
for (i in 1:ncol(df2)){
...
```{r}
# Print Street Names before applying Hunspell Corrections
...
We have 117 mispelt words caught by the `hunspell` function.
...
As you can see, there are some road names like Bleecker Street, which we know are legitimate NY street names but they get flagged out as misspelling because the word "Bleecker" is not in the Hunspell English dictionary GB and US. One way to address this is the `Spelling` package.
...
```{r}
# Get and clean full Manhattan Street Dictionary (from Chang)
...
streetDStreetNamesClean %>% length()
streetDStreetNamesClean %>% sort() %>% head()
```
